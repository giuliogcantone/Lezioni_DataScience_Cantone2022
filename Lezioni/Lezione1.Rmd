...
Formato: è l'estensione (es, `.txt`) dei documenti (files) con cui lavoriamo.
...
- Essenzialmente ci sono due formati di files per *conservare codice* su R.
...
- In alto a sinistra, andate su "Files",
- Poi su "New Files",
- Infine "R Script".
...
Per lanciare un comando dentro uno script bisogna posizionare il cursore del testo all'inizio del comando e poi premere `Invio + ctrl`. 
...
In queste lezioni impareremo a **disambiguare** le operazioni per ottenere il risultato desiderato.
...
Se premuto, apre un *chunk* del linguaggio scelto. Adesso si può scrivere un codice dentro il chunk, ad esempio una sequenza di comandi. In alto a destra del chunk c'è una freccia verso destra. Premendo la freccia, il codice dentro il chunk sarà eseguito.
...
Se la cartella del Markdown è in una sottocartella della cartella progetto e la cartella in cui conservate i files dati salvati o da importare sta su una sottocartella diversa, R non riuscirà a torvare i files da caricare.
...
Però spesso R *interpreta* anche `=` come un comando di assegnazione. Spesso la grammatica corretta dipende dal dialetto che stiamo usando.
La pratica ci farà capire quale sia il simbolo più appropriato.
...
Come abbiamo già visto, **quando si scrive il termine dell'oggetto** e si esegue il codice, viene eseguito un `print` implicito dell'oggetto. Questa operazione viene chiamata "invocazione" oppure chiamata (call) di un oggetto, vedremo nelle prossime lezioni" che è utile "concatenerare" una chiamata ad una vista con:
...
Nel caso di valori logici è corretto parlare di "Interrogazione" rispetto ad una proposizione. Per esempio il comando:
...
```
interroga se la stringa "capitale" è contenuto nella stringa "Roma è la capitale" e poi nella stringa "Io vivo a Roma".
...
Ci sono diversi modi per salvare TUTTO il workspace come unico file di memoria.
...
Un modo alternativo di "salvare il workspace" è il comando:
